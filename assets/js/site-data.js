window.SITE = {

  /* =========================================================
     LANGUAGE
     ========================================================= */

  defaultLang: "en",


  /* =========================================================
     SITE IDENTITY
     ========================================================= */

  siteTitle: {
    en: "Little Boat's Backyard",
    zh: "小船的后花园"
  },

  ownerName: "Jingzhou Hao",
  ownerNameZh: "郝泾舟",

  tagline: {
    en: "I know that if I wait, it will be lost, gone.",
    zh: "如果我等待的话，一切都将消失无踪。"
  },


  /* =========================================================
     HOME COVER
     ========================================================= */

  homeCoverDesktop:
    "assets/images/cover.jpeg",

  homeCoverMobile:
    "assets/images/cover.jpeg",

  homeCoverAlt: {
    en: "Website cover",
    zh: "网站封面"
  },


  /* =========================================================
     PHOTOGRAPHY PAGE
     ========================================================= */

  photography: {

    cover:
      "assets/images/cover.jpeg",

    coverPosition:
      "50% 48%",

    intro: {

      en:
        "A growing set of small photo stories. Some are organized by place, some by time, and some simply by the feeling that made them belong together.",

      zh:
        "一些慢慢积累下来的摄影故事。有些按地点整理，有些按时间整理，也有些只是因为某种共同的感觉而被放在一起。"
    },

    heading: {

      en:
        "Photographs, grouped by place, time, or whatever held them together.",

      zh:
        "按地点、时间，或某种把它们联系在一起的感觉整理的照片。"
    }
  },


  /* =========================================================
     BIO PAGE
     ========================================================= */

  bio: {

    cover:
      "assets/images/cover.jpeg",

    coverPosition:
      "50% 48%",

    portrait:
      "assets/images/bio/Bio_cover.jpg",

    portraitAlt: {

      en:
        "Portrait of Jingzhou Hao",

      zh:
        "Jingzhou Hao 的个人照片"
    },

    portraitPosition:
      "50% 50%",


    paragraphs: {

      en: [

        "A film photography beginner, blues harmonica amateur, a researcher who has too many hobbies, human manic. I likes to find the human perspective on people, the moment that is imperfect but real.",

        "What else can I say......I hope everyone can find the happiness they deserve."
      ],

      zh: [

        "一个胶片摄影初学者，不专业蓝调口琴手，不务正业的研究人员。努力寻找人类的一面，不完美但是真实的一面。",

        "我能说啥呢......希望所有人都能找到他们值得拥有的快乐。"
      ]
    },


    cvFile:
      "",

    cvDescription: {

      en:
        "A PDF copy of my CV is available here.",

      zh:
        "这里可以下载我的 PDF 版简历。"
    }
  },


  /* =========================================================
     SOCIAL
     ========================================================= */

  social: {

    instagram:
      "https://www.instagram.com/jingzhou_hao/",

    linkedin:
      "https://www.linkedin.com/in/jingzhou-hao-2b9927434/"
  },


  /* =========================================================
     COPYRIGHT
     ========================================================= */

  copyright: {

    en:
      "All rights reserved.",

    zh:
      "保留所有权利。"
  },


  /* =========================================================
     COLLECTIONS
     ========================================================= */

  collections: [


    /* =========================================================
       BACK TO HUMAN
       回到人类
       ========================================================= */

    {

      id:
        "back-to-human",

      titleZh:
        "回到人类",

      title:
        "Back to Human",

      meta:
        "",


      cover:
        "assets/images/Back_to_human/000020.jpg",

      coverPosition:
        "50% 50%",


      intro: {

        en:
          "The name of this collection was inspired by Amy Winehouse's album <Back to Black>. Its Chinese name is 《黑色会》, though I think 《回到黑暗》—Back to Darkness—would be a better translation. I don't know why, but something in Amy's voice always makes me want to care about people, to get close to human desire and emotion. Honestly, I'm already tired of so-called AI art. Just this once, can we return to people—and care about human again?",

        zh:
          "这个合集的名字灵感来源于Amy Winehouse 的专辑<Back to Black>。中文名是《黑色会》，我觉得更恰当的名字是《回到黑暗》。不知为什么，Amy的歌声总是让我想关爱人类，接触人类的欲望和情绪。老实说我已经厌倦所谓的AI艺术了，就这次，把注意力重新聚焦在人类身上，做回人类，OK？"
      },


      photos: [


        /* 01 */

        {

          src:
            "assets/images/Back_to_human/000020.jpg",

          caption: {

            en:
              "Having conversation by the Charles River.",

            zh:
              "在查尔斯河畔促膝长谈"
          },

          note:
            "Shot with Kodak Funsaver"
        },


        /* 02 */

        {

          src:
            "assets/images/Back_to_human/000017-2.jpg",

          caption: {

            en:
              "A cyclist caught in passing. The light and shadow in this moment felt just right.",

            zh:
              "抓拍骑自行车的人，这一刻的光影很妙"
          },

          note:
            "Shot with Kodak Funsaver"
        },


        /* 03 */

        {

          src:
            "assets/images/Back_to_human/000020140009-1.jpg",

          caption: {

            en:
              "A person sitting on the stairs, and geese looking for food.",

            zh:
              "公园阶梯上沉思的人和揾食的鹅"
          },

          note:
            "Shot with Kodak h35, film is gold 200"
        },


        /* 04 */

        {

          src:
            "assets/images/Back_to_human/000020140020-1.jpg",

          caption: {

            en:
              "The Green Line C train passing through the woods.",

            zh:
              "greenlineC线从林中穿过"
          },

          note:
            "Shot with kodak h35, film is gold 200"
        },


        /* 05 */

        {

          src:
            "assets/images/Back_to_human/000020140061-1.jpg",

          caption: {

            en:
              "Children playing on the swings.",

            zh:
              "荡秋千的孩子们和他们的父母"
          },

          note:
            "Shot with kodak h35, film is gold 200"
        },


        /* 06 */

        {

          src:
            "assets/images/Back_to_human/000020140063-1-4.jpg",

          caption: {

            en:
              "A hide-and-seek point of view.",

            zh:
              "捉迷藏视角"
          },

          note:
            "Shot with kodak h35, film is gold 200"
        },


        /* 07 */

        {

          src:
            "assets/images/Back_to_human/Hao_004.jpg",

          caption: {

            en:
              "Nostalgia-core.",

            zh:
              "怀旧核"
          },

          note:
            "Shot with kodak daylight single use camera"
        },


        /* 08 */

        {

          src:
            "assets/images/Back_to_human/cover.jpeg",

          caption: {

            en:
              "",

            zh:
              ""
          },

          note:
            ""
        }

      ]
    },



    /* =========================================================
       CARNIVAL
       那一天 / Ce jour-là
       ========================================================= */

    {

      id:
        "Ce jour-là",

      titleZh:
        "那一天",

      title:
        "Ce jour-là",

      meta:
        "",


      cover:
        "assets/images/Carnival/000407460027-2.jpg",

      coverPosition:
        "50% 50%",


      intro: {

        en:
          "Ce jour-là is a photobook by the French photographer Willy Ronis, translated into Chinese as 《那一天》—That Day. The book brings together some of Ronis's finest humanist photographs. His images capture the warmth and playfulness of everyday life, with an almost Voltairean affection for the ordinary world. He once said, “If I wait, everything will disappear without a trace.” The soul of photography lies in holding on to a moment, allowing us to remember that day forever—one day unlike every other.",

        zh:
          "《Ce jour-là》是法国摄影师Willy Ronis的摄影集，中文译名叫《那一天》。这本书集和Ronis人文摄影的佳作，他的照片捕捉了了生活中的人情味和俏皮，透露出一种伏尔泰式的对世俗生活的热爱。他说过，“如果我等待的话，一切都将消失无踪”。摄影的灵魂在于定格某个瞬间，从而让我们永远记住那一天，和每一天都不一样。"
      },


      photos: [


        /* 01 */

        {

          src:
            "assets/images/Carnival/000018.jpg",

          caption: {

            en:
              "Musicians playing on Newbury Street. They looked so young—probably students from Berklee College of Music.",

            zh:
              "Newbury st上演奏的音乐家，他们那么年轻，看起来是Berklee音乐学院的学生"
          },

          note:
            "Shot with Kodak Funsaver"
        },


        /* 02 */

        {

          src:
            "assets/images/Carnival/000407460027-2.jpg",

          caption: {

            en:
              "A ceramics stall at the street market. I was trying to take a candid shot, but the vendor noticed me. I almost wanted to run away, but she told me I could photograph as much as I liked. She has such a beautiful smile. She looks so healthy, so full of life.",

            zh:
              "街头集市上的陶艺摊子，本来想偷拍结果被摊主发现了。心虚的我有点想跑掉，但是她说我可以随便拍。她笑地真美，她看着真健康"
          },

          note:
            "Shot with Pentax espio 928, Film is ektar100"
        }

      ]
    },



    /* =========================================================
       MOMENTS OF SUMMER
       夏天还没过去，我已经开始怀念它了
       ========================================================= */

    {

      id:
        "moments-of-summer",

      titleZh:
        "夏天还没过去，我已经开始怀念它了",

      title:
        "Moments of Summer 2026",

      meta:
        "",


      cover:
        "assets/images/Moments_of_summer/Hao024-1.jpg",

      coverPosition:
        "50% 50%",


      intro: {

        en:
          "Summer always gives me a bittersweet feeling. Everything is growing, people are playing, and even the sun seems to shine with an energy it never has at any other time. But at the same time, I know it will all eventually be washed over by rain. Every summer in a life happens only once.",

        zh:
          "夏天总是给我一种苦甜参半的感觉，一边是万物在生长，人类在玩耍，连太阳都用前所未有的活力照耀大地，但是又知道这些总会被雨水覆盖，生命里的每一个夏天都是唯一的。"
      },


      photos: [


        /* 01 */

        {

          src:
            "assets/images/Moments_of_summer/000020140008-1.jpg",

          caption: {

            en:
              "Beer garden near fenway park. Seriously, how many games do the Red Sox play in one summer? It feels like there are barely any days in the week when I can get through the Fenway Park area normally.",

            zh:
              "比赛日的露天酒吧。请问一个夏天Red Sox到底要打几场比赛？感觉一周里没几天我可以正常从Fenway park附近通过的。"
          },

          note:
            "Shot with Kodak h35, film is gold 200"
        },


        /* 02 */

        {

          src:
            "assets/images/Moments_of_summer/000020140055-1.jpg",

          caption: {

            en:
              "so good......so good!!",

            zh:
              "so good......so good!!"
          },

          note:
            "Shot with Kodak h35, film is gold 200"
        },


        /* 03 */

        {

          src:
            "assets/images/Moments_of_summer/000024.jpg",

          caption: {

            en:
              "Street art at golden hour.",

            zh:
              "黄金时刻的街头涂鸦"
          },

          note:
            "Shot with Kodak Funsaver"
        },


        /* 04 */

        {

          src:
            "assets/images/Moments_of_summer/Hao024-1.jpg",

          caption: {

            en:
              "Endless Summer.",

            zh:
              "无尽夏"
          },

          note:
            "Shot with Kodak Daylight single use camera"
        },


        /* 05 */

        {

          src:
            "assets/images/Moments_of_summer/Hao_009.jpg",

          caption: {

            en:
              "Shimmering water.",

            zh:
              "水光潋滟"
          },

          note:
            "Shot with Kodak Daylight single use camera"
        }

      ]
    },



    /* =========================================================
       SECRET TRAIN STATION
       秘境车站
       ========================================================= */

    {

      id:
        "secret-train-station",

      titleZh:
        "秘境车站",

      title:
        "Hikyō station",

      meta:
        "",


      cover:
        "assets/images/secret_train_station/000407460024-1.jpg",

      coverPosition:
        "50% 50%",


      intro: {

        en:
          "I don't know why, but deserted train stations always make me want to take pictures. There is something about them that feels forgotten in a corner of the world—or maybe it's that they make me imagine all the unknown places I could reach if I boarded a train there.",

        zh:
          "不知道为啥荒凉的车站总是能唤起我拍照的冲动，有一种在世界的角落里被遗忘的感觉，或者怎么说呢......会让我肖想从这个车站出发能够到达的未知的地方，车站常常和可能性联系在一起。"
      },


      photos: [


        /* 01 */

        {

          src:
            "assets/images/secret_train_station/000407460024-1.jpg",

          caption: {

            en:
              "A Green Line train sitting on the tracks during the summer shutdown.",

            zh:
              "暑假绿线停运，停靠在露天铁轨上的车厢"
          },

          note:
            "Shot with Pentax espio 928, Film is ektar100"
        },


        /* 02 */

        {

          src:
            "assets/images/secret_train_station/000407470024-1.jpg",

          caption: {

            en:
              "I took this through the window as the Blue Line passed the station. It looked like a ruin left behind by the industrial age.",

            zh:
              "蓝线经过这个站的时候隔着玻璃拍了一张，看着很有工业时代的废墟的感觉"
          },

          note:
            "Shot with Pentax espio 928, Film is Ultramax400"
        }

      ]
    }

  ]
};
